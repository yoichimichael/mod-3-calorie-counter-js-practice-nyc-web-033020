...
You can find the code for the backend API in the `./calorie-counter-api` directory. *We should not have to change anything*. To get it up and running, make sure you migrate and then seed the database and run using `rails s`. To see all routes available for this API, run `rails routes` inside the API directory.
